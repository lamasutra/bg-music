...
func TestPatterns(t *testing.T) {
	...
	playerShotDownRegExp = *regexp.MustCompile(fmt.Sprintf(playerShotDownPatternTemplate, nickname))
	...
	if !playerShotDownRegExp.MatchString(strKill) {
		t.Error("player kill pattern is invalid")
	}
	if playerShotDownRegExp.MatchString(strAnyKill) {
		t.Error("player kill pattern is invalid")
	}
