...
func (p *Player) LoadData(entity *client.Entity, nearestAirfield *client.Entity, state *client.State, indicators *client.Indicators, mapInfo *client.MapInfo, mapObj *client.MapObj) {
	...
	p.IsDrone = !indicators.Valid && (state != nil && state.IsValid())
	...
	fmt.Println("player: dead:", p.Dead, "spawned:", p.Spawned, "ground:", p.IsGround, "air:", p.IsAir, "drone:", p.IsDrone)
...
// @todo count with airfield shape and vector
...
func (p *Player) CheckIsSpawned(spawnPoint *client.Entity, mapInfo *client.MapInfo, mapObj *client.MapObj) {
	// fmt.Println("afd: ", mapObj.GetDistance(p.CurrentEntity, airfield, mapInfo))
	dist := mapObj.GetDistance(p.CurrentEntity, spawnPoint, mapInfo)
	p.Spawned = p.LastComputedSpeed.value == 0 && p.IsGround && dist < 0.1
	fmt.Println("check is spawned", dist)
}
